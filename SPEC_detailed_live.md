...
- Active races use lightweight `race_snapshot` messages for display cars/timing at the normal snapshot rate, while controller clients receive focused `controller_feedback` messages for their own car, phone feedback, server clock sync, and server-derived countdown marks. Full room/lobby state is sent less frequently during racing. Lobby/results still use full room state. The 3D scene smooths server snapshots locally for cars, cockpit, and camera.

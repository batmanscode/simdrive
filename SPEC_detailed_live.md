...
- Five original tracks are present: `Sakura Sprint`, `Alpine Grand Prix`, `Fjord Loop`, `Keys Causeway`, and `Cloudline Ascent`.
- Server-authoritative simplified racing physics: throttle, brake, steering, per-vehicle tuning, per-driver setup multipliers where available, optional warm-up/flying start timing, velocity-based lateral slip, aero/speed drag, downforce-style speed-building grip, elevation/grade acceleration effects for uphill/downhill sections, optional gentle stability assist, tuned surface grip/drag, rain grip reduction, tuk-tuk rollover risk, wall slowdown, directional car contact/crash handling, optional crash/off-track reset, checkpoint-gated lap finish, DNF handling, and results.
- Active races use lightweight `race_snapshot` messages for display cars/timing at the normal snapshot rate, while controller clients receive focused `controller_feedback` messages for their own car, own crash events, compact nearby-rival audio descriptors, nearby non-self crash events, server clock sync, and server-derived countdown marks. Full room/lobby state is sent less frequently during racing. Lobby/results still use full room state. The 3D scene smooths server snapshots locally for cars, cockpit, and camera.
- 3D cockpit-style race view with procedural vehicle-specific silhouettes, formula nose/front tyres/compact wheel display, kart floor/front wheels/mounted data logger, centered stock-truck POV with broad hood/cowl and subtle digital speedometer, tuk-tuk canopy/handlebars/small analog speedometer, tuk-tuk `TIP RISK` cockpit toast, per-driver Hands/Paws/None cockpit cosmetic toggle, simple binary visual front-wheel steering, proportional cockpit steering wheel/hands/paws movement, visually spinning tyres based on travelled wheel distance, smooth generated track ribbons, rubbered-in road detail, static and live skid marks, raised curbs, racing line, grass/runoff, camera shake, HUD, in-race minimap, global race leaderboard, countdown lights, a short first-place banner, and display-side auto-spectate for finished racers while others are still active.
- When a race pane's owning player finishes and other racers are still active, the display keeps the finish view briefly, then after about 2.5 seconds follows the leading active racer. Finished players can switch between active racers, return to their own finished car with `My finish`, and re-enter spectate with `Watch`. This is display-side UI state only and does not change server race state or the WebSocket protocol.
- `CARS.md` tracks the current vehicle roster, setup availability, research anchors, visual notes, and handling parameters.
